...
- `backend/Dockerfile`をマルチステージビルド化（JDKビルド→JRE実行）
...
### セキュリティ（Security）
- Cloudinary、SendGrid、Stripe のAPIキーをdocker-compose.ymlからの完全除去（要キーローテーション）
...
| v4.2 | 追加機能（Cloudinary・管理者作成API） | TBD | ⏳ 未着手 |
| v4.3 | 追加機能（D&D並び替え） | TBD | ⏳ 未着手 |
| v4.4 | 追加機能（マークダウンエディタ） | TBD | ⏳ 未着手 |
| v5.0 | テスト実装 | TBD | ⏳ 未着手 |
| v5.1 | セキュリティ対策 | TBD | ⏳ 未着手 |
| v5.2 | 本番デプロイ | TBD | ⏳ 未着手 |
