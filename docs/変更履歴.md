...
### 変更（Changed）
- ユーザーページOGP画像をアバター全面表示デザインに変更
  - アバターが1200×630全面を覆い、左上にDisplayNameをテキストシャドウ付きで大きく配置
  - PRO/STUDIOはaccent色バッジ（右上）+ 「pic-me.net」表記
  - FREE/STARTERは「PicMe」ロゴ（右下）
  - アバターなし時はLP風デザイン（ライムグリーン背景 + 幾何学装飾）にDisplayNameを中央配置
- LP用OGP画像のキャッチコピーをLPヒーローに統一（「つくる人の、見せる場所。」+ PicMe）
...
### 追加（Added）
- OGP画像の動的生成（Next.js `ImageResponse` / Satori）
  - LP・一般ページ用の静的OGP画像（ライムグリーン背景 + マガジン風デザイン）
  - ユーザーページ用の動的OGP画像（プラン別デザイン差別化）
    - FREE/STARTER: 白背景、320pxアバター、シンプルデザイン
    - PRO/STUDIO: ヘッダー画像全面背景 or テーマカラー背景、400pxアバター、白太枠+オフセットシャドウ、PRO/STUDIOバッジ、accent色幾何学装飾
    - アバターなし時: グラデーション背景に巨大頭文字表示
  - Twitter Card画像も同一デザインで生成（`twitter-image.tsx`）
- フォント取得ユーティリティ `og-fonts.ts`（Outfit Bold + Noto Sans JP Bold、Google Fonts TTF）

### 変更（Changed）
- OGP画像をFile Convention方式（`opengraph-image.tsx`）に移行
- `layout.tsx`のmetadata `images`指定を削除（File Conventionで自動解決）
- ユーザーページのTwitter Cardを `summary` → `summary_large_image` に変更
...
### 修正（Fixed）
- ユーザーページのOGP URLが `picme.com` 固定だった致命的バグを修正（`pic-me.net` + 環境変数対応）
- ユーザーページにcanonical URLを追加

### 追加（Added）
- ルートlayout.tsxにOGP画像（`og-image.png`）とURL設定を追加
- ルートlayout.tsxのTwitterカードに画像設定を追加
- ログインページにメタデータ追加（title: "ログイン | PicMe"）
- 新規登録ページにメタデータ追加（title: "新規登録 | PicMe"）

### 注意（Notes）
- OGP画像ファイル（`frontend/public/og-image.png`、1200x630推奨）の配置が別途必要
