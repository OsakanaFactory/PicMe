...
export default function ArtworksPage() {
  ...
  const openEditDialog = (artwork: Artwork) => {
    ...
    reset({ title: artwork.title, description: artwork.description || '', imageUrl: artwork.imageUrl });
  ...
  const handleCreate = async () => {
    ...
    setIsSaving(true);
    try {
      await uploadArtwork(uploadFile, { title: uploadTitle || uploadFile.name.replace(/\.[^.]+$/, ''), description: uploadDescription }, (percent) => setUploadProgress(percent));
      setIsCreateDialogOpen(false);
      fetchArtworks();
    } catch (error: any) {
      alert(error.response?.data?.message || '作品のアップロードに失敗しました');
    } finally {
      setIsSaving(false);
      setUploadProgress(0);
    }
  ...
  const onEditSubmit = async (data: EditFormValues) => {
    ...
    setIsSaving(true);
    try {
      await updateArtwork(editingArtwork.id, { ...data, description: data.description || '' });
      setIsEditDialogOpen(false);
      fetchArtworks();
    } catch (error: any) {
      alert(error.response?.data?.message || '作品の保存に失敗しました');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        icon={ImageIcon}
        title="作品管理"
        description={`ポートフォリオに表示する作品を追加・編集します${artworkLimit > 0 ? `（${artworks.length}/${artworkLimit}）` : ''}`}
      >
        {artworks.length > 1 && (
      ...
      <UpgradePrompt currentCount={artworks.length} limit={artworkLimit} itemName="作品" />

      {artworks.length === 0 ? (
        <motion.div
          className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-slate-300 rounded-lg bg-white"
          initial="hidden"
          animate="visible"
        >
          ...
          <h3 className="font-outfit font-bold text-lg text-slate-900">作品がありません</h3>
          <p className="text-sm text-slate-500 mt-2 mb-6 max-w-sm">
            最初の作品をアップロードして、あなたのクリエイティビティを世界に公開しましょう。
          </p>
          <Button onClick={openCreateDialog} variant="outline">作品を追加する</Button>
        </motion.div>
      ) : isReorderMode ? (
        <SortableArtworkList
          artworks={artworks}
            ...
            <motion.div key={artwork.id} variants={dashStaggerItem}>
              ...
              >
                ...
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-slate-500 truncate mt-1">{artwork.description || '説明なし'}</p>
                </CardContent>
                <CardFooter className="p-4 pt-0 flex justify-end gap-2">
                  <Button variant="ghost" size="sm" onClick={() => openEditDialog(artwork)}>
                    <Pencil className="h-4 w-4 mr-1" /> 編集
                  </Button>
                  <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => handleDelete(artwork.id)} disabled={isDeleting === artwork.id}>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>画像をアップロードして作品を追加します。</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <FileUpload onFileSelect={(file) => setUploadFile(file)} onRemove={() => setUploadFile(null)} uploading={isSaving} progress={uploadProgress} />
            <div className="space-y-2">
              <Label htmlFor="uploadTitle">タイトル</Label>
              <Input id="uploadTitle" placeholder="作品のタイトル" value={uploadTitle} onChange={(e) => setUploadTitle(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="uploadDescription">説明文 (任意)</Label>
              <Textarea id="uploadDescription" placeholder="作品についての説明" className="resize-none h-24" value={uploadDescription} onChange={(e) => setUploadDescription(e.target.value)} />
            </div>
            <DialogFooter className="pt-4">
              <Button type="button" variant="outline" onClick={() => setIsCreateDialogOpen(false)}>キャンセル</Button>
              <Button onClick={handleCreate} isLoading={isSaving} disabled={!uploadFile}>アップロード</Button>
            </DialogFooter>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>作品の情報を編集します。</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit(onEditSubmit)} className="space-y-4 py-4">
            {editingArtwork && (
              <div className="aspect-video w-full bg-slate-100 rounded-lg overflow-hidden">
                <img src={editingArtwork.imageUrl} alt={editingArtwork.title} className="w-full h-full object-contain" />
              </div>
            )}
            <div className="space-y-2">
              <Label htmlFor="title">タイトル</Label>
              <Input id="title" placeholder="作品のタイトル" {...register('title')} error={errors.title?.message} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="description">説明文 (任意)</Label>
              <Textarea id="description" placeholder="作品についての説明" className="resize-none h-24" {...register('description')} error={errors.description?.message} />
            </div>
            <DialogFooter className="pt-4">
              <Button type="button" variant="outline" onClick={() => setIsEditDialogOpen(false)}>キャンセル</Button>
              <Button type="submit" isLoading={isSaving}>更新する</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
